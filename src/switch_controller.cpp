  ...
  try{
    switch(ctrl_mode){
      ...
      case CTRLMODE_JOINT_IMP:
        ...
        init_fn = boost::bind(&JointSpaceImpCtrl::init, &ctrl_js_imp);
        loop_fn = boost::bind(&JointSpaceImpCtrl::loop, &ctrl_js_imp, _1);
        stop_fn = boost::bind(&JointSpaceImpCtrl::stop, &ctrl_js_imp);
